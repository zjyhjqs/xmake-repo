package("xerces-c")
    ...
    local is_system_transcoder_supported = is_plat("linux") or is_plat("windows") or is_plat("mingw") or is_plat("macos")
    ...
    on_load(function (package)
        ...
        elseif package:config("network_accessor") == "cfurl" then
            assert(package:is_plat("macos"), "macOS only")
            add_frameworks("CoreServices")
        elseif package:config("network_accessor") == "winsock" then
            assert(package:is_plat("windows") or package:is_plat("cygwin") or package:is_plat("mingw"), "Windows, Cygwin, MingGW only")
            add_syslinks("advapi32")
        end
    end)
